...
describe("buildCreateWorkbookTransaction — 纯 SurrealQL 构造", () => {
  ...
  test("实体表挂 record_activity event：行 CREATE/UPDATE/DELETE 自动落 activity_event", () => {
    ...
    // CREATE/UPDATE → record.write，DELETE → record.delete
    ...
    // UPDATE 也作为次周真实业务更新证据。
    expect(sql).toMatch(/WHEN \$event = "CREATE" OR \$event = "UPDATE" OR \$event = "DELETE"/);
  });
