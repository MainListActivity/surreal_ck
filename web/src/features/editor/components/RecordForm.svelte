<script lang="ts">
  ...
  // 日期输入仍用只读 stub（DatePicker 迁移不在 07e 范围，留后续簇），不阻断其它字段提交。
  ...
  {#each columns as col}
    ...
    <label class="field" class:span-all={!dense && isLongText(col)}>
      ...
      {:else if col.fieldType === "date"}
        <!-- 日期选择器（DatePicker）随 07e 接入；此处只读展示当前值。 -->
        <span class="stub-input" title="日期编辑器即将上线（07e）">
          {values[col.key] == null || values[col.key] === "" ? "（日期选择即将上线）" : String(values[col.key])}
        </span>
