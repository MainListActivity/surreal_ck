...
{#if editorUi.showDataCheck}
  <div class="overlay" role="presentation"><div class="dialog" role="dialog" aria-modal="true" aria-labelledby="data-check-title">
    <header><div><h2 id="data-check-title">全范围数据体检</h2><p>检查全部数据表的必填与格式规则，不受当前 500 条视图窗口限制</p></div><button class="icon" aria-label="关闭" onclick={() => (editorUi.showDataCheck = false)}><X size={18} /></button></header>
    <div class="body">
      {#if result}
        ...
        {#if result.findings.length}<ul class="findings">{#each result.findings as finding}<li><div><strong>{finding.category === "required" ? "必填" : "格式"} · {finding.field}</strong><span>{finding.explanation} · 规则 {finding.ruleKey}</span><small>{finding.sheetId} / {finding.recordId}</small></div><button onclick={() => void locate(finding.sheetId, finding.recordId)}>定位记录</button></li>{/each}</ul>{/if}
