<script lang="ts">
  ...
  import type { FilterOp, GridColumnDef } from "@surreal-ck/shared/rpc.types";
  import { cleanFilterDrafts, filterValueKind, type FilterDraft } from "./tool-drafts";
  ...
  async function apply() {
    const cleaned = cleanFilterDrafts(
      drafts,
      editorStore.columns.map((c) => c.key),
    );
...
<div class="tool-panel">
  ...
  {#each drafts as draft (draft.id)}
    ...
    {@const valueKind = filterValueKind(draft.op)}
    <div class="row">
      <SelectMenu
        compact
        value={draft.key}
