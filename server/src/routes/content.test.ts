...
describe("platform content routes", () => {
  test("ordinary authenticated users cannot read platform content without an entitlement session", async () => {
    ...
    expect(result.error.code).toBe("content_authorization_unavailable");
  });
