...
export function createDashboardDraftIntent(input: CreateDashboardDraftInput): DashboardDraftIntent {
  ...
  if (!description) throw new ServiceError("VALIDATION_ERROR", "缺少仪表盘需求描述");
  if (!input.workspaceId) throw new ServiceError("VALIDATION_ERROR", "缺少工作区上下文");
  if (input.schemas.length === 0) throw new ServiceError("VALIDATION_ERROR", "没有可用的数据表 schema");
