...
describe("SurrealMemoryStorage（绑定调用者 surrealSession）", () => {
  ...
  beforeEach(() => {
    session = fakeSession();
    storage = new SurrealMemoryStorage(() => ({ db: session as never, subject: "user:test" }));
  });
  ...
  test("没有任何子 store 依赖已退役的 getLocalDb（构造时不连任何全局连接）", () => {
    ...
    expect(() => new SurrealMemoryStorage(() => ({ db: session as never, subject: "user:test" }))).not.toThrow();
  });
});
...
describe("SurrealObservabilityStorage（retention 走 env，不查 app_setting）", () => {
  test("createSpan 用注入会话写入，过期时间来自 env retention 天数", async () => {
    ...
    const storage = new SurrealObservabilityStorage(() => ({ db: session as never, subject: "user:test" }), { retentionDays: 7 });
    const now = Date.now();
...
describe("SurrealMastraStore（组合三个会话绑定子 store）", () => {
  test("用同一个注入 resolver 组合 memory / workflows / observability", () => {
    const session = fakeSession();
    const store = new SurrealMastraStore(() => ({ db: session as never, subject: "user:test" }));
