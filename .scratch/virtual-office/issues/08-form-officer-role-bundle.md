...
1. 收到派单"基于 X 数据表或待确认结构方案的字段缺口创建并发布表单"
...
- [ ] 在 data-analyst 已产出结构方案且相关数据表已由授权发起人的 admin session 落库后，form-officer 能产出可访问的 share URL
...
- VO-07 的 DDL 执行策略已定：DDL 使用发出指令者的当前 workspace session。若数据表仍未落库，本 issue 只能基于结构方案产出表单草稿，不能发布绑定真实数据表的表单。
