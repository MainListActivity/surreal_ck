...
把 `web/legacy/`（即原 `src/renderer/`）的 Svelte 5 + RevoGrid 前端迁到 `web/`，构建工具改为标准 Vite 5；用 oidc-client-ts 做 SPA OIDC（**不经过后端**）；用 `surrealdb` 浏览器 SDK 直连 SurrealDB 做所有读 / 写 / LIVE；workspace 切换走 IdP；AI 抽屉接 Bun server `/api/chat` + `/api/chat/stream`；管理员"创建 workspace"由前端拿 NS-admin token 后直接执行 `DEFINE DATABASE` + 模板。**不引入新 UI 功能**。
...
- `pnpm --filter @surreal-ck/web dev` 起 Vite dev server，访问 `localhost:5173` 能：
  ...
  - 从 IdP token claim 拿到 current_db + role
  - 用 `surrealdb` 浏览器 SDK `db.signin({ ac, ns, db, token })` 直连 SurrealDB
  - 进入某 workspace 看到工作簿主界面（RevoGrid 表格直接从 SurrealDB SELECT / LIVE）
  - 切换 workspace 触发 IdP silent refresh → 拿新 token → 重新 signin
  - 管理员可点"新建 workspace" → 调 IdP → 拿 NS-admin token → 前端执行 DEFINE DATABASE + 模板
  - 打开 AI 抽屉，发消息，看到 Router workflow 流式响应（调后端 /api/chat）
- `web/legacy/` 被删空。

## 风险

- **vite.config.ts 重写**：原配置面向 Electrobun 的 views:// 协议，需要换成标准 Vite 5 web 配置。
- **OIDC 登录壳**：MVP 直接用 `oidc-client-ts` 或类似 SPA OIDC 库；token 放 sessionStorage（接受 XSS 风险，因为后端是唯一信任域）。
...
| # | 名称 | 主体 | 依赖 |
|---|---|---|---|
| 01 | Vite 5 + Svelte 5 web 构建链 | 重写 web/vite.config.ts、web/index.html、web/src/main.ts | wp-restructure |
| 02 | OIDC 登录壳（SPA，不经过后端） | oidc-client-ts + PKCE + token / claim 管理 + silent refresh | 01 |
| 03 | SurrealDB 直连客户端 | web/src/lib/surreal.ts：按 token.current_db + token.role 调 db.signin；workspace-store 暴露当前 db 连接 | 02 |
| 04 | API client（Hono RPC，仅 Mastra） | web/src/lib/api.ts，对接 /api/chat 等少量后端 endpoint | 01 |
| 05 | Workspace 切换器 | 顶栏 dropdown 列 IdP token 中的 available_workspaces；切换触发 IdP silent refresh + 重新 signin | 02, 03 |
| 06 | 新建 workspace 流程 | 调 IdP create-workspace → 拿 NS-admin token → 前端执行 DEFINE DATABASE + 应用 shared/sql/workspace-template/ | 03, 05 |
