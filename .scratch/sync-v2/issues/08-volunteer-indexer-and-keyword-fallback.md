...
- [ ] **不引入分布式 claim/lease**：允许多客户端偶发同时处理同一条 `pending`，依靠 `(resource, profile_key)` 稳定 ID 的 `UPSERT` 幂等收敛，按 ADR §10 接受偶发重复 embedding API 调用作为简化代价。
- [ ] 回归测试覆盖 `pending → indexed/failed`、重复 sweep 的幂等收敛、并发多客户端处理同一 `pending` 的最终一致性，和关键词 fallback。
