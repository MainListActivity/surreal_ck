...
- [x] 在线启动或手动触发重建时，固定共享表会从 remote 全量拉取并落到本地结构影子库。
- [x] 清空本地结构影子库后再次重建，结果可恢复且重复执行不产生额外脏状态。
- [x] 至少一条现有高层读路径在重建完成后仍从本地影子读取，不直接改成 remote 查询。
- [x] 回归测试覆盖重建顺序、幂等性和远端不可达时的失败表述。
