...
本 PRD 的产品语义仍保留：全局 AI 入口、上下文快照、Router workflow、子 agent、确认后写入、暂停/恢复等仍是当前方向。

但本文中 Electrobun 主进程、renderer RPC、secret-store、独立 sidecar 窗口等执行形态已经被 [`docs/adr/web-only-pivot.md`](../../docs/adr/web-only-pivot.md) 和 [`docs/adr/frontend-direct-connect.md`](../../docs/adr/frontend-direct-connect.md) 推翻。后续执行以 `.scratch/mastra-router-migration/` 与 `.scratch/web-frontend-migration/` 为准：AI 抽屉在 Web 前端中运行，LLM/Router workflow 在 Bun server 中运行，业务数据读写默认由浏览器直连 SurrealDB。
...
构建一个完整的 agent 产品层。用户只从全局 AI 窗口进入 AI 能力；表格、仪表盘、文档等页面不再各自提供独立 AI 入口。AI 通过 Mastra agent 编排受控工具，不直接任意操作数据库或页面，而是输出结构化意图：读操作由 Router workflow 以调用者 workspace session 执行，写操作先回到 Web 前端让用户确认，再按当前架构走浏览器直连 SurrealDB 或明确的 Bun server 窄 endpoint。

第一阶段提供页面级 AI 入口，形成所有页面都能访问的 AI 抽屉。后续在这个入口之上接入 Mastra agent，优先完成导航与查找、AI 创建仪表盘、当前行债权分析三个闭环。

AI 入口不应以模态遮罩阻塞主应用。当前长期形态是 Web app 内的右侧 drawer / panel：它不阻塞主内容点击、滚动和输入，不依赖原生第二窗口。

全局 AI 窗口需要像 Codex 对话上下文一样展示当前上下文提示。用户选中某个 sheet 时，对话框展示 `工作簿名 / Sheet 名`；用户选中某一行时，对话框展示 `Sheet 名 / 主要字段组合`，例如 `债权申报表 / 张三 || ZQ-2026-001 || ent_claim:abc`。用户发起对话时，当前选中的 workbook、sheet、row 摘要和 record id 必须随消息一起发送给 Bun server 的 Router workflow。
...
4. As an authenticated user, I want the AI window to behave as an attached right-side drawer or panel, so that it feels connected to the current workspace without behaving like a modal.
5. As an authenticated user, I want a future desktop wrapper, if any, to reuse the same Web AI surface, so that desktop packaging does not fork AI behavior.
...
35. As an administrator, I want AI actions to go through existing database permissions and current-session execution paths, so that security remains enforced by the same boundaries as manual work.
...
37. As an administrator, I want model provider API keys to remain server-side, so that AI features do not expose secrets to the browser.
38. As a developer, I want AI capabilities represented by typed HTTP/WS contracts, so that Web frontend and Bun server integration remains maintainable.
39. As a developer, I want Mastra tools to reuse shared domain modules and caller SurrealDB sessions rather than duplicate business logic, so that manual and AI workflows share validation.
...
42. As a developer, I want record patch proposals to use the browser direct SurrealDB path or an explicitly scoped server endpoint after confirmation, so that field constraints and serialization remain consistent with the current architecture.
...
- Treat the AI surface as a Web app panel/drawer. The old Electrobun sidecar direction is cancelled; a future desktop shell, if needed, should wrap the same Web app rather than reintroduce a second native AI window.
- Persist panel open/closed and layout preferences only within the Web app surface; do not depend on native window placement.
...
- Keep LLM keys and Router workflow execution behind the Bun server. The Web frontend sends user messages and receives structured responses; business data reads/writes still follow the current architecture: browser direct SurrealDB for product data, server-side workflow/tool calls using the caller's signed-in workspace session.
...
- Add an AI chat/session HTTP + WS contract that supports sending a message with route context, workbook context, sheet context, selected row context, and optional dashboard context.
- Add explicit confirmation contracts for actions such as navigation, saving a dashboard view, or applying a row patch; execution should use browser direct SurrealDB where possible and only use narrow server endpoints for server-owned work.
...
- Avoid putting permission filters into generated frontend queries. Data access must continue to be enforced by schema permissions, access type, and current workspace session.
...
1. Global AI drawer/panel and context plumbing.
2. Workspace-db workflow run persistence (`WorkflowsStorage` over `workflow_run`) — required before any multi-step or suspendable agent flow is built.
3. AI chat HTTP/WS contract with streaming progress channel; a temporary single NavigationAgent proves end-to-end plumbing.
