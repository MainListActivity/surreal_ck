...
## 2026-06-10 triage 收口

> *This was generated by AI during triage.*

本 PRD 脱离 needs-triage 悬空状态，标记 **done（historical）**：产品语义已全部交付或重新归属，本文不再是任何执行路径的依据。归属映射：

- **已交付**：AI 抽屉骨架、上下文快照 DTO、Router workflow + 四子 agent、workflow_run 持久化、暂停/恢复、`/api/chat` + WS stream（issues 01–05、08、10–12，服务端形态见 D1 簇）。
- **仍开放（已改写为 post-pivot issue）**：
  - `issues/06`（重写）— 行分析提案卡逐字段确认 + 直连写入（前端半边）。
  - `issues/07`（重写）— 上下文快照测试套件 + Web 选择态接线。
  - `.scratch/web-dashboard-migration/issues/05-ai-dashboard-draft-persist.md`（D3-05，新立）— AI 图表草稿预览 → 确认 → 直连持久化。
- **领域内容**：债权场景的模板、Excel 导入、agent 领域提示移交 `.scratch/claims-vertical/PRD.md`（法律内容数据库可配置、可整体移除）。
- 本文执行形态描述（Electrobun、`ai.executeAction`、主进程服务）仅作历史参考，后续 agent 不得据此开工。
