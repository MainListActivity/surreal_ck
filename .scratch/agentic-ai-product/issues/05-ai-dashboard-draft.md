...
>
> 2026-06-10 post-pivot 注记：本 issue 保持 done，但其验收清单的"预览卡片 / 确认保存 / widget 渲染"是 Electrobun legacy 形态，已随 `web/legacy` 退役。当前仍然存活的是服务端半边（`inspectSchema` + `generateDashboardDraft` 挂在 dashboardAgent 上，D1 簇迁入）。**前端"草稿预览 → 确认 → 直连持久化"链路重新立项为 `.scratch/web-dashboard-migration/issues/05-ai-dashboard-draft-persist.md`（D3-05）**，依赖 D3 dashboard 数据层与 D2-08 抽屉。不要按本 issue 的旧文案恢复 `ai.executeAction` / 主进程保存服务。
