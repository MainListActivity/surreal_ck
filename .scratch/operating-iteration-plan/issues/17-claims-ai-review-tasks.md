...
- [ ] 模板包可提供行分析领域背景、字段语义和检查重点，平台 prompt 中不新增法律硬编码。
- [ ] 选中债权记录后可生成包含申报金额、审核状态、材料情况和引用依据的摘要。
- [ ] 风险清单区分缺失信息、金额异常、疑似重复和期限风险，并说明判断依据。
- [ ] 待办建议可以形成写入提案，但只有用户确认后才创建或更新记录。
- [ ] 有关联资源时回答展示 citation；无资源时明确区分台账事实和模型建议。
- [ ] 非模板工作簿及无领域提示模板继续使用通用行分析，不出现债权措辞。
- [ ] 测试使用 fake 模型验证 instructions 装配、确认边界及无模板回退，不调用真实模型。
