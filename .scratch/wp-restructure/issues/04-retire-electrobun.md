...
- [x] `grep -ri electrobun .` 在源码 / 配置中无命中（legacy 目录里的引用允许保留，因为内容还没搬完）。
- [x] `pnpm install` 成功后 node_modules 不含 electrobun 包。
- [x] CI（如果已就绪）通过；如果 CI 还没有，本 issue 仅要求"不再尝试调 Electrobun"。
