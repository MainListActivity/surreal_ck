...
│       • IdP 不写本表；IdP 只在登录时调本应用的 default-scope hook 读取默认 token scope。
│       • 用户登录与切换都查本表：登录 hook 选默认 scope；switch-workspace 校验 membership。
│       • dispatcher 启动时遍历 status='active' 的 workspace 确定有哪些 ws db 要服务的虚拟员工。
│       • Reconciler 周期性校对本表与各 ws db 的 user 表是否漂移（参见 §8 成员管理写路径）。
│
└── database: ws_<auto-generated-id>      — 每 workspace 一个 db，db 名由系统自动生成（见 §2.1）
      ...
      │     AUTHENTICATE {
      │       -- IdP token 中带标准 claim：
      │       --   $token["https://surrealdb.com/db"] = 当前 workspace db 名
      │       --   $token["https://surrealdb.com/ac"] = 'admin' | 'participant'
      ...
      │       };
      │       -- TODO(簇 C issue 02)：确认 SurrealDB AUTHENTICATE 的真实触发时机。
      │       --   若 per-SIGNIN 触发，下面的 user upsert + last_seen_at 更新可保留；
      │       --   若 per-query 触发，每条 SELECT 都会触发 UPDATE，写放大 + 污染 CHANGEFEED，
      │       --   届时需把 last_seen_at 更新挪出 AUTHENTICATE（改 heartbeat endpoint 或 dispatcher 级批写）。
      │       --   并发首次访问可能撞 user.subject 唯一索引，需用 ON DUPLICATE KEY UPDATE upsert。
